const searchItems = [
  ...
  {title:'Lúcifer — play the game',description:'Tony’s browser recreation of the hidden-answer computer prank. English and Portuguese.',href:'/lucifer/',keywords:'lucifer lúcifer game play brazil brasil brazilian brasileiro portuguese portugues português dos prank secret hidden answer spirit nostalgia'},
  ...
  {title:'Things I’ve built',description:'Explore my apps, projects, and the Lúcifer game.',href:'#work',keywords:'portfolio work projects developing software things built'}
];
...
input.addEventListener('keydown',event=>{if(event.key==='ArrowDown'){event.preventDefault();results.querySelector('a')?.focus();}if(event.key==='Enter'){const first=results.querySelector('a');if(first){dialog.close();location.href=first.href;}}});
